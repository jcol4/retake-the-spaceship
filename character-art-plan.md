...
> ## ⚠️ HALF OF THIS IS SUPERSEDED — read this first
>
> This was written for a **generated 3D model** (`tools/gen_soldier.py`) on a rigged character.
> The game is now presented isometrically with **hand-drawn 2D sprites** — see
> [`docs/presentation-direction.md`](docs/presentation-direction.md). The generator script, the
> Mixamo rig and every `.glb` this plan targets have been deleted.
>
> | Still governs the art | Describes nothing that exists |
> |---|---|
...
> things that can be.
>
> Item 3's outline shader is not merely unnecessary now, it is inapplicable: the characters
> *are* drawn, so whatever outline they have is drawn too. §4.1's cadence numbers survive as
> the **authoring contract** for the walk cycle — the sprite is drawn to read at 4.5 m/s rather
> than the speed being fitted to a clip.
