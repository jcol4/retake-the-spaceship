...
- **Muzzle flashes emit no light.** They used to: a transient, unshadowed, room-filling `OmniLight3D` timed to the shot's muzzle animation frame. It was removed once the flash became drawn art — a frame of the `fire_shoot` sprite on its own `flash` layer, sitting on the barrel where the gun actually is. A 10-energy white flood over a 25 m range washed out the whole room *including the flash it was selling*, and the thing it was standing in for now draws itself.
